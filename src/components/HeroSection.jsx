...
function HeroSearchForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Advanced: different username per platform */}
      <div>
        <button
          type="button"
            ...
            {[...activePlatforms].filter(id => id !== "google").map(id => {
              ...
              return (
                <div key={id} className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-md flex items-center justify-center text-white flex-shrink-0 text-xs ${
                    id === "google" ? "bg-white border border-gray-200" : pl.color
                  }`}>
                    <PlatformIcon id={id} className="w-3 h-3" />
                  </div>
                  <span className="text-xs text-gray-500 w-20 flex-shrink-0">{pl?.label}</span>
                  <input
                    type="text"
